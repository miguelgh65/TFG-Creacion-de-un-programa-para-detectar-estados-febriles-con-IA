...
if __name__ == "__main__":
    ...
    while cap:
        ...
        for detection in detections:
            if detection is not None:
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                    a1 = torch.tensor([x1], device='cuda')
                    
                    a2 = torch.tensor([x2], device='cuda')
                   
                    b1 = torch.tensor([y1], device='cuda')
                    
                    b2 = torch.tensor([y2], device='cuda')
                    cord1=int(a1.item())
                    ord1=int(b1.item())
                    cord2=int(a2.item())
                    ord2=int(b2.item())
        ...
        if opt.webcam==1:
            ...
            cv2.imshow('frame2', RGBimg)
            
            if(ord1!=100 and ord2!=200 and cord1!=100 and cord2!=200):
            	print("a")
            	
            	out.write(Convertir_BGR(RGBimg))
            	cv2.imshow('frame3', RGBimg[ord1:ord2,cord1:cord2])
            	cord1,cord2,ord1,ord2=50,75,50,75
